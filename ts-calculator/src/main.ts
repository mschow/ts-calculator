...
export class Main {
    ...
    private assignEventListeners(): void {
        this.calculatorDOM.buttonContainer?.addEventListener(
            'click',
            (event) => {
    ...
    private calcNumberSelect(value: string): void {
        if (this.activeEquation.rightOperator.length >= this.MAX_CHARACTERS)
            return;
        this.activeEquation.rightOperator =
    ...
    private calculateTotal(): void {
        if (
            ...
            const left: number =
                parseFloat(this.activeEquation.leftOperator) || 0;
